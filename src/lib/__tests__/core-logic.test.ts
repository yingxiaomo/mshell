...
import { parseTarget } from "../quickConnect";
import {
...
describe("parseTarget (quick connect)", () => {
  it("parses user@host:port", () => {
    expect(parseTarget("bob@example.com:2222")).toEqual({ username: "bob", host: "example.com", port: 2222 });
  });
  it("defaults user to root and port to 22", () => {
    expect(parseTarget("example.com")).toEqual({ username: "root", host: "example.com", port: 22 });
    expect(parseTarget("bob@example.com")).toEqual({ username: "bob", host: "example.com", port: 22 });
  });
  it("ignores a non-numeric trailing colon segment", () => {
    expect(parseTarget("root@host:abc")).toEqual({ username: "root", host: "host:abc", port: 22 });
  });
