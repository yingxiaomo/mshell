...
import { clearTerminalBuffers, stashScrollback } from "../lib/events";
...
function createSessionsStore() {
  ...
  addTab: (result) => {
    ...
    const { showMonitor } = useUiStore.getState();
    if (!showMonitor) {
  ...
  closeTab: async (sessionId) => {
    ...
    try {
      await cmd(commands.sessionClose, { sessionId });
    } catch {
  ...
  markConnected: (oldSessionId, newSessionId, newChannelId, result) => {
    ...
    if (nextId !== oldSessionId) {
      ...
      // Reconnect produced a new session id — carry the old scrollback over so the
      // new terminal replays it (also frees the old id's buffers, no orphan leak).
      stashScrollback(oldSessionId, nextId);
    }
    set((s) => {
  ...
  reconnectTab: async (sessionId) => {
    ...
    try {
      ...
      try {
        result = await cmd(commands.sessionReconnect, { sessionId, cols, rows });
      } catch {
        // Session may already be gone from the manager; open by connection.
        result = await cmd(commands.sessionOpen, { connectionId: tab.connectionId, cols, rows });
      }
      // Tab may have been closed while awaiting.
      if (!get().tabs.some((t) => t.sessionId === sessionId)) {
        try {
          await cmd(commands.sessionClose, { sessionId: result.sessionId });
        } catch {
          /* ignore */
        }
        return;
