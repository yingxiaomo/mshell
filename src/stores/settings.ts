...
function createSettingsStore() {
  ...
  load: async () => {
    ...
    try {
      const raw = await cmd(commands.getSettings);
      ...
      if (
        ...
        void cmd(commands.saveSettings, { settings }).catch((e) => console.warn(e));
      }
    } catch (e) {
  ...
  save: async (next) => {
    ...
    try {
      ...
      const settings = normalizeSettings(await cmd(commands.saveSettings, { settings: normalized }));
      applyChrome(chromeMode(settings.theme));
  ...
  clearCredentials: async () => {
    set({ error: null });
    try {
      await cmd(commands.clearAllCredentials);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      set({ error: msg });
      throw e;
