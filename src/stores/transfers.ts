...
function createTransfersStore() {
  return create<TransfersState>((set, get) => ({
    ...
    cancel: async (transferId) => {
      try {
        await cmd(commands.transferCancel, { transferId });
      } catch {
        /* already finished */
      }
      set((s) => ({
    ...
    retry: async (transferId) => {
      ...
      if (item.direction === "upload") {
        const newId = await cmd(commands.sftpUpload, {
        ...
        const newId = await cmd(commands.sftpDownload, {
          sessionId: item.sessionId,
          remotePath: item.remotePath,
          localPath: item.localPath,
        });
