...
/** A trusted host key entry (known_hosts.json). */
...
/** Result of generating a keypair via ssh-keygen. */
...
/** Sidebar activity views — now extensible via features/registry.ts. */
export type SideViewId = string;
...
export interface Connection {
  ...
  /** Command auto-run in the shell right after connect (per-connection login script). */
  onConnect?: string | null;
  /** Connection color tag for visual identification (CSS color hex, e.g. "#ef4444"). */
  color?: string | null;
}
...
export interface RemoteEntry {
  ...
  /** Unix permission bits (& 0o7777) if reported. */
  mode?: number | null;
}
