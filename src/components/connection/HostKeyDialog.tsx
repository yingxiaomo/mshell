...
export function HostKeyDialog({ prompt, onClose }: HostKeyDialogProps) {
  ...
  async function handleTrust() {
    ...
    setOpenError(null);
    try {
      await cmd(commands.hostKeyTrust, { host: prompt.host, fingerprint: prompt.fingerprint });
      const { cols, rows } = estimateTerminalGeometry();
      const result = await cmd(commands.sessionOpen, { connectionId: prompt.connectionId, cols, rows });
      addTab(result);
