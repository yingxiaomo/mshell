...
export function TunnelsView() {
  ...
  const refresh = useCallback(async (sessionId: string) => {
    try {
      const list = await cmd(commands.tunnelList, { sessionId });
      setLive(list);
      setError(null);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    // 每 5 秒自动刷新状态
    const timer = setInterval(() => void refresh(active.sessionId), 5000);
    return () => clearInterval(timer);
  }, [active?.sessionId, active?.disconnected, refresh]);

  useEffect(() => {
    // Capture the session id the effect is bound to, rather than closing over
    // the whole `active` object (which isn't in the dep array → could go stale).
    const sid = active?.sessionId;
    if (!sid) return;
    let cancelled = false;
    const unsub = bus.on("tunnel-status", (ev) => {
      if (cancelled) return;
  ...
  async function onStart(config: TunnelConfig) {
    ...
    setBusyId(config.id);
    try {
      await cmd(commands.tunnelStart, { sessionId: active.sessionId, config });
  ...
  async function onStop(tunnelId: string) {
    ...
    setBusyId(tunnelId);
    try {
      await cmd(commands.tunnelStop, { sessionId: active.sessionId, tunnelId });
      await refresh(active.sessionId);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally { setBusyId(null); }
  ...
  async function onStartAll() {
    if (!active) return;
    for (const r of rows) {
      if (r.live?.state === "running") continue;
      setBusyId(r.config.id);
      try {
        await cmd(commands.tunnelStart, { sessionId: active.sessionId, config: r.config });
      } catch (e) {
        setError(e instanceof Error ? e.message : String(e));
      }
    }
  ...
  async function onStopAll() {
    ...
    for (const r of rows) {
      ...
      setBusyId(r.config.id);
      try {
        await cmd(commands.tunnelStop, { sessionId: active.sessionId, tunnelId: r.config.id });
      } catch (e) {
        setError(e instanceof Error ? e.message : String(e));
      }
    }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between gap-2 border-b border-zinc-800 px-3 py-2">
        <span className="text-xs text-zinc-500">会话：{active.name}</span>
        <div className="flex items-center gap-1.5">
          <button type="button" disabled={busyId !== null} onClick={onStartAll} className="rounded border border-zinc-700 px-2 py-1 text-[11px] text-zinc-300 hover:bg-zinc-800 disabled:opacity-40">全部启动</button>
          <button type="button" disabled={busyId !== null} onClick={onStopAll} className="rounded border border-zinc-700 px-2 py-1 text-[11px] text-zinc-300 hover:bg-zinc-800 disabled:opacity-40">全部停止</button>
        </div>
      </div>
      {error && (
