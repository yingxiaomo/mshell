...
import { consumeGotoLine } from "../../stores/ui";
import {
  SearchQuery,
  findNext,
  findPrevious,
...
export function FileEditor({
  ...
  // ── Auto-save state (refs, not state, to avoid scheduling races) ──
  ...
  const mountEditor = useCallback(
    (
      ...
      emitDirty(false);
      // Jump to a specific line after the editor renders.
    ...
    [emitDirty, scheduleAutoSave, clearAutoSave],
  );
  ...
  useEffect(() => {
    ...
    void cmd(commands.sftpReadText, { sessionId, remotePath }).then(
      (b64) => {
        if (cancelled || !editorRef.current) return;
        const tabId = `${sessionId}::${remotePath}`;
        const gotoLine = consumeGotoLine(tabId);
        mountEditor(b64Decode(b64), codeTheme, appChrome, remotePath, gotoLine);
        setLoading(false);
      },
      (err) => {
        if (cancelled) return;
  ...
  const handleSave = useCallback(async () => {
    ...
    const baseline = baselineRef.current;
    // Skip save if content hasn't changed since the last save.
    if (content === baseline) {
    ...
    try {
      let lastErr: unknown;
      for (let attempt = 1; attempt <= SAVE_RETRY_MAX; attempt++) {
        try {
          await cmd(commands.sftpWriteText, {
            sessionId,
            remotePath,
      ...
      // Leave dirty state so user knows the save didn't stick.
      emitDirty(true);
      isSavingRef.current = false;
      setSaving(false);
    }
    ...
    >
      <div className="flex shrink-0 items-center justify-between border-b border-zinc-800 px-3 py-1">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {error && <span className="text-xs text-red-400" title="保存失败">{error}</span>}
          {saving && (
            <span className="text-xs text-sky-400 animate-pulse">保存中…</span>
          )}
          {!saving && lastSaved && !error && (
            <span className="hidden sm:inline text-[10px] text-zinc-600" title={`最后保存：${lastSaved.toLocaleTimeString()}`}>
