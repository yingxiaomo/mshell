...
export function TransferBar() {
  ...
  useEffect(() => {
    const unsub = bus.on("transfer-progress", (ev) => {
      ...
      // 速度计算
      const now = Date.now();
      const prev = prevBytesRef.current.get(ev.transferId);
      if (prev && ev.bytes > prev.bytes) {
        const dt = (now - prev.time) / 1000;
        if (dt > 0.5) {
          const bps = (ev.bytes - prev.bytes) / dt;
          setSpeeds((s) => {
            const next = new Map(s);
            next.set(ev.transferId, formatBytes(bps) + "/s");
            return next;
          });
          prevBytesRef.current.set(ev.transferId, { bytes: ev.bytes, time: now });
        }
      } else {
        prevBytesRef.current.set(ev.transferId, { bytes: ev.bytes, time: now });
      }
      // 完成通知（仅一次）
      const prevStatus = prevStatusRef.current.get(ev.transferId);
      if (ev.status === "done" && prevStatus !== "done") {
        showToast(`传输完成：${ev.transferId.slice(0, 8)}`, "success");
      } else if (ev.status === "failed" && prevStatus !== "failed" && ev.error) {
        showToast(`传输失败：${ev.error}`, "error");
      }
      prevStatusRef.current.set(ev.transferId, ev.status);
    });
    return () => unsub();
  }, [applyProgress]);
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="shrink-0 tabular-nums text-zinc-500">
                    ...
                    {item.status === "running" && speeds.has(item.transferId) && (
                      <span className="ml-1 text-zinc-600">· {speeds.get(item.transferId)}</span>
                    )}
                  </span>
                  <span
                    className={clsx(
