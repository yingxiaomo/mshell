...
pub fn map_core_err(err: CoreError) -> String {
    let client = match err {
        ...
        // Map network failures by structured io::ErrorKind rather than parsing
        // libssh2/OS error text (which varies by locale and version).
        CoreError::Io(ref e) => {
            use std::io::ErrorKind;
            let message = match e.kind() {
                ErrorKind::ConnectionRefused => {
                    "连接被拒绝。目标端口可能未开放，或服务未启动。".to_string()
                }
                ErrorKind::TimedOut => {
                    "连接超时。请检查主机地址、端口与网络连通性。".to_string()
                }
                ErrorKind::ConnectionReset => {
                    "连接被对方重置。可能是中间设备断开或服务异常。".to_string()
                }
                ErrorKind::ConnectionAborted => {
                    "连接被中断，可能是服务异常或网络波动。".to_string()
                }
                ErrorKind::AddrNotAvailable | ErrorKind::AddrInUse => {
                    "本地地址不可用或被占用。".to_string()
                }
                // Unknown kind → fall back to text-based humanization.
                _ => humanize_message(&e.to_string()),
            };
...
pub(crate) fn humanize_message(raw: &str) -> String {
    ...
    if lower.contains("unable to exchange")
        ...
        || lower.contains("algorithm negotiation")
    {
        return "SSH 密钥协商失败。对方 SSH 版本或算法可能不兼容，请尝试在服务器端升级 OpenSSH 或在连接设置中调整兼容选项。".into();
    }
    if lower.contains("unable to connect to agent") || lower.contains("agent pipe") {
        return "SSH Agent 认证失败：无法连接 Windows SSH 代理。请在 PowerShell 中执行 'Start-Service ssh-agent' 启动 OpenSSH 认证代理服务，或在连接设置中改用密码/密钥认证。".into();
    }
    if (lower.contains("publickey") || lower.contains("public key"))
        && (lower.contains("invalid") || lower.contains("unverified") || lower.contains("rejected"))
    {
        return "公钥认证失败：服务器拒绝了该密钥。请确认远程服务器 ~/.ssh/authorized_keys 中已添加对应的公钥，且私钥文件路径和口令正确。".into();
    }
    if lower.contains("banner") {
        return "未收到有效的 SSH 标识。端口可能不是 SSH 服务，或者该服务器需要 SSH 协议版本 2（不支持 SSH1）。".into();
    }
    if lower.contains("protocol") && lower.contains("ssh") {
        return format!("SSH 协议错误：{raw}");
    }
    if lower.contains("disconnect")
        && (lower.contains("protocol error") || lower.contains("protocol_version_not_supported"))
    {
        return "SSH 协议版本不兼容。请检查服务器是否支持 SSH 协议版本 2。".into();
    }
